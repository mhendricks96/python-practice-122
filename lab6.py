...
# That's a good amount of cat food to feed your cat!
...
def getSingleCanCost(catName: str):
    ...
    """
    while True:
        cost = v.get_real("Cost for a can of cat food for " + catName + ": ")
        if cost >= 0.0:
            return cost
        else:
            print("It can't cost negative! Please enter a valid number.")
...
def askToContinue():
    ...
    """
    answer = v.get_y_or_n("Would you like to add another cat? (y/n): ")
    return answer
...
def getMonthlyCansUsed(catName: str):
    ...
    """
    while True:
        cansUsed = v.get_integer("Cans of cat food used per month for " + catName + ": ")
        if cansUsed >= 0:
            return cansUsed
        else:
            print("how does a cat eat negative cans?! Please enter a valid number.")
...
def getCatName():
    """
    Prompts user to enter cat name
    :return: catName as str
    """
    name = v.get_string("Your cats name: ")
    return name
